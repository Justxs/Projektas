...
<form action="include/procCreate.php" method="POST">
    ...
    <input name="date" type="date" class="form-control" min="<?= date('Y-m-d') ?>" required oninvalid="this.setCustomValidity('Įveskite išvykimo datą')" oninput="this.setCustomValidity('')" />

    <label class="mt-2 form-label">Leidžiamas bagažo svoris:</label>
    <input name="price" type="number" class="form-control" min="1" step="any" placeholder="0" required oninvalid="this.setCustomValidity('Įveskite išvykimo datą')" oninput="this.setCustomValidity('')" />
